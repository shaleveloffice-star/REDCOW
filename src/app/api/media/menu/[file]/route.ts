...
import {
  MENU_UPLOAD_DATA_DIR,
  MENU_UPLOAD_PUBLIC_DIR
} from "@/lib/admin/save-menu-image";

export const runtime = "nodejs";

const SAFE_FILE = /^img-\d+-[a-z0-9]+\.(jpe?g|png|webp|gif)$/i;
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const candidates = [
    path.join(MENU_UPLOAD_DATA_DIR, fileName),
    path.join(MENU_UPLOAD_PUBLIC_DIR, fileName)
  ];
